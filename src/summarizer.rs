...
use ptree::{print_tree, TreeBuilder, Style};
...
pub fn generate_summary(
    ...
    let input_dir_canonicalized = input_dir.canonicalize().unwrap_or_else(|_| input_dir.to_path_buf());
    ...
    writeln!(writer, "{}", String::from_utf8_lossy(&tree_string)).context("Failed to write tree structure")?;
    ...
    for pattern in exclude_patterns {
        builder.filter_entry(move |entry| {
            !entry
                .path()
                .to_str()
                .map(|s| glob::Pattern::new(pattern).map(|p| p.matches(s)).unwrap_or(false))
                .unwrap_or(false)
        });
    }
    ...
    for result in walker {
        ...
        if entry.file_type().map_or(false, |ft| ft.is_file()) {
            ...
            if !parent_path.as_os_str().is_empty() {
                tree_builder.add_child(parent_path.to_string_lossy().to_string(), item_name);
            } else {
                tree_builder.add_child(base_dir.to_string_lossy().to_string(), item_name);
            }
        }
    }
...
fn add_path_to_tree(
    ...
    if path.is_dir() {
        ...
        tree_builder.add_child(parent_path, file_name.to_string());
    }
}
    ...
    for component in dir_path.components() {
        ...
        if !added_dirs.contains(&full_current) {
            ...
            if prev_path.as_os_str().is_empty() {
                tree_builder.add_child(base_dir.to_string_lossy().to_string(), component_name.to_string());
            } else {
                let prev_str = prev_path.to_string_lossy().to_string();
                tree_builder.add_child(prev_str, component_name.to_string());
            }
...
fn process_file_contents(
    ...
    for path in file_paths {
        ...
        match std::fs::read_to_string(path) {
            ...
            Err(err) => {
                writeln!(writer, "Error reading file: {}", err)
...
fn print_tree_to_writer(writer: &mut impl Write, tree: &ptree::item::StringItem) -> Result<()> {
    ...
    ptree::write_tree_with(tree, writer, &config)
...
fn write_statistics(
    ...
    writeln!(writer, "Total directories: {}", stats.total_directories).context("Failed to write statistics")?;
    writeln!(writer, "Total lines of code: {}", stats.total_lines).context("Failed to write statistics")?;
    ...
    if !stats.extension_counts.is_empty() {
        ...
        for (ext, count) in extensions {
            let ext_name = if ext.is_empty() { "[no extension]" } else { ext };
            writeln!(writer, "  {}: {} files", ext_name, count).context("Failed to write statistics")?;
        }
    }
    ...
    if !stats.extension_lines.is_empty() {
        ...
        for (ext, lines) in extensions {
            let ext_name = if ext.is_empty() { "[no extension]" } else { ext };
            writeln!(writer, "  {}: {} lines", ext_name, lines).context("Failed to write statistics")?;
        }
    }
